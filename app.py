...
# ---------------------------------
st.set_page_config(
    page_title="Campus Brain | AI Learning",
    page_icon="🎓",
    layout="wide"
)
...
if not st.session_state.authenticated:
    ...
    with st.container():
        ...
        st.markdown(f"<h1 class='grad-text' style='text-align:center;'>{'Create Account' if st.session_state.auth_mode == 'register' else 'Welcome Back'}</h1>", unsafe_allow_html=True)
        
        if st.session_state.auth_mode == "register":
            name = st.text_input("Full Name", placeholder="John Doe")
            email = st.text_input("University Email", placeholder="name@college.edu")
            password = st.text_input("Password", type="password")
            confirm_pw = st.text_input("Confirm Password", type="password")
            college = st.selectbox("Institution", ["Engineering College", "Tech Institute", "Science University"])
...
# ---------------------------------
    ...
    if documents: doc_embeddings = model.encode(documents, convert_to_tensor=True)

    # --- Sidebar Nav ---
    with st.sidebar:
        st.markdown(f"<h2 class='grad-text'>Campus Brain</h2>", unsafe_allow_html=True)
        st.write(f"Logged in as: **{st.session_state.user_name}**")
        st.markdown("---")
        
        menus = {"🏠 Dashboard": "welcome", "🔍 Search": "search", "📚 Library": "library", "📈 Progress": "recommend", "💬 AI Chat": "chat"}
    ...
    if st.session_state.screen == "welcome":
        ...
        with c1: st.markdown('<div class="db-card"><b>Topics Mastered</b><br><h2 style="color:#6366f1">14</h2></div>', unsafe_allow_html=True)
        with c2: st.markdown('<div class="db-card"><b>Study Hours</b><br><h2 style="color:#6366f1">42.5</h2></div>', unsafe_allow_html=True)
        with c3: st.markdown('<div class="db-card"><b>Learning Rank</b><br><h2 style="color:#6366f1">#8</h2></div>', unsafe_allow_html=True)
    ...
    # --- SCREEN: SEARCH ---
        ...
        query = st.text_input("", placeholder="Ask anything about your syllabus...")
    ...
    # --- SCREEN: RESULTS ---
        ...
        st.button("← Back", on_click=lambda: setattr(st.session_state, 'screen', 'search'))
    ...
    # --- SCREEN: CHAT ---
        ...
        u_input = st.chat_input("Ask a follow-up...")
        if u_input:
            with st.chat_message("user"): st.write(u_input)
            with st.chat_message("assistant"): st.write("That is a great question regarding your current study material. In professional environments, this is implemented using...")
